...
{
    template <typename T>
    TaoListModel<T>::TaoListModel(QObject *parent) : Super(parent)
    {
    }

    template <typename T>
    TaoListModel<T>::TaoListModel(const QList<T> &nodeList, QObject *parent) : Super(parent), m_nodeList(nodeList)
    {
    }
